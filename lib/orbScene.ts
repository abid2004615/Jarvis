...
import { ORB_MODE_SETTINGS } from "@/lib/jarvisState";
...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  let rafId = 0;
  let disposed = false;
  let currentMode: OrbMode = "IDLE";
  let audioLevel = 0;
  let hudVisible = true;

  function applyModeSettings(mode: OrbMode) {
    currentMode = mode;
    const settings = ORB_MODE_SETTINGS[mode];
    if (typeof settings === "undefined") return;
    bloom.strength = settings.bloom + audioLevel * 1.3;
    chromaticPass.uniforms.uIntensity.value = 0.0025 + settings.pulse * 0.0018 + audioLevel * 0.0025;
    (scanRing1.material as THREE.MeshBasicMaterial).opacity = 0.1 + settings.scanline * 0.18;
    (scanRing2.material as THREE.MeshBasicMaterial).opacity = 0.08 + settings.scanline * 0.14;
    coreSphereMat.opacity = 0.08 + settings.coreBrightness * 0.1;
    icoWireMat.opacity = 0.45 + settings.coreBrightness * 0.25;
    orbGroup.rotation.y += 0.05;
  }

  function setMode(mode: OrbMode) {
    applyModeSettings(mode);
  }

  function setAudioLevel(level: number) {
    audioLevel = THREE.MathUtils.clamp(level, 0, 1.2);
  }
  ...
  function animate() {
    ...
    outerShell.rotation.y += dynamicRot;
    outerShell.rotation.x = Math.sin(t * 0.08) * 0.05 + audioLevel * 0.4;

    panelGroup.rotation.y += modeSettings.rotationSpeed * 1.2;
    panelGroup.rotation.x = Math.sin(t * 0.08 + 0.5) * 0.04 + audioLevel * 0.25;

    shell2.rotation.y -= modeSettings.rotationSpeed * 0.85;
    shell2.rotation.z = Math.sin(t * 0.12) * 0.03 + audioLevel * 0.1;

    innerCore.rotation.y -= modeSettings.rotationSpeed * 3.2;
    innerCore.rotation.z += 0.002 + audioLevel * 0.03;
    innerCore.rotation.x = Math.cos(t * 0.1) * 0.08 + audioLevel * 0.2;

    const pulseWave = Math.sin(t * (1.4 + modeSettings.pulse * 0.7) + audioLevel * 6);
    const surge = Math.max(0, Math.sin(t * (0.45 + modeSettings.pulse * 0.25))) ** 3;
    const coreScale = 1 + surge * (0.5 + modeSettings.pulse * 0.45) + pulseWave * 0.06 + audioLevel * 0.24;
    coreSphere.scale.setScalar(coreScale);
    glowSphere.scale.setScalar(1 + surge * 0.9 + audioLevel * 0.3);
    icoWire.scale.setScalar(1 + surge * 0.6 + audioLevel * 0.2);
    icoWireMat.opacity = THREE.MathUtils.clamp(0.45 + settingsPulse(modeSettings.pulse, audioLevel), 0.35, 1);

    coreSphereMat.opacity = THREE.MathUtils.clamp(0.06 + modeSettings.coreBrightness * 0.14 + audioLevel * 0.2 + surge * 0.25, 0.06, 0.75);
    glowSphereMat.opacity = THREE.MathUtils.clamp(0.02 + modeSettings.pulse * 0.08 + audioLevel * 0.18, 0.02, 0.4);

    debris.forEach((d) => {
      const u = d.userData as DebrisOrbit;
      const a = t * (u.speed * (0.5 + modeSettings.particleSpeed * 0.85)) + u.phase;
      d.position.set(
        u.orbitR * Math.cos(a) * Math.cos(u.tiltX),
        u.orbitR * Math.sin(u.tiltX) * Math.sin(a * 0.8) + Math.sin(a * 0.3 + u.tiltZ) * 0.2 + audioLevel * 0.3,
        u.orbitR * Math.sin(a) * Math.cos(u.tiltZ),
      );
      d.rotation.x += 0.014 + audioLevel * 0.04;
      d.rotation.z += 0.012 + audioLevel * 0.03;
      const material = d.material as THREE.MeshBasicMaterial;
      material.opacity = 0.25 + Math.sin(a * 3 + u.phase) * 0.15 + audioLevel * 0.35;
    });
    ...
    for (const [group, mult] of driftGroups) {
      group.children.forEach((sp) => {
        const sprite = sp as THREE.Sprite;
        const u = sprite.userData as SpriteDrift;
        u.theta += u.speed * mult * (1 + modeSettings.particleSpeed * 0.8);
        sprite.position.set(
        ...
        material.opacity = 0.25 + Math.sin(t * 3 + u.theta * 3) * 0.2 + audioLevel * 0.35;
      });
    }

    const scanY1 = Math.sin(t * (0.35 + modeSettings.ringSpeed * 0.8)) * R1;
    scanRing1.position.y = scanY1;
    const scanS1 = Math.sqrt(Math.max(0, R1 * R1 - scanY1 * scanY1)) / R1;
    scanRing1.scale.set(scanS1, scanS1, 1);
    (scanRing1.material as THREE.MeshBasicMaterial).opacity = (0.12 + modeSettings.scanline * 0.18) * (0.5 + audioLevel * 0.8);

    const scanY2 = Math.sin(t * (0.6 + modeSettings.ringSpeed * 1.2) + 2) * R3;
    scanRing2.position.y = scanY2;
    const scanS2 = Math.sqrt(Math.max(0, R3 * R3 - scanY2 * scanY2)) / R3;
    scanRing2.scale.set(scanS2, scanS2, 1);
    (scanRing2.material as THREE.MeshBasicMaterial).opacity = (0.08 + modeSettings.scanline * 0.14) * (0.6 + audioLevel * 0.8);

    dustPoints.rotation.y += 0.0002 + modeSettings.particleSpeed * 0.0003 + audioLevel * 0.0005;

    flickerTimer += 0.016;
    if (flickerTimer > 0.1) {
      flickerTimer = 0;
      panelGroup.children.forEach((p) => {
        if (Math.random() > 0.94 + modeSettings.pulse * 0.02) {
          p.visible = !p.visible;
        }
      });
    }

    bloom.strength = modeSettings.bloom + Math.sin(t * 0.8) * 0.4 + audioLevel * 1.3;
    chromaticPass.uniforms.uTime.value = t;

    orbGroup.visible = hudVisible;
    controls.update();
    composer.render();
  }

  function settingsPulse(base: number, audio: number) {
    return 0.18 + base * 0.18 + audio * 0.22;
  }
